			...
			<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
				{projects.map((project) => (
					<button
						key={project.id}
						type="button"
						className="glass-card-light gpu-accelerated p-4 flex flex-col gap-3 cursor-pointer text-left w-full"
					...
					>
						...
						<p className="text-[var(--muted)] text-sm m-0 line-clamp-2">
							{project.description}
						</p>
						<div className="flex gap-2 flex-wrap">
							{project.tech.slice(0, 3).map((tech) => (
								<span key={tech} className="chip-light text-xs">
									{tech}
								</span>
							))}
							{project.tech.length > 3 && (
								<span className="chip-light text-xs">
