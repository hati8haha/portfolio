...
const SkillsPuzzleGame = () => {
	...
	const calculateFinalScore = useCallback(
		(completionTime: number, baseScore: number) => {
	...
	const handlePieceClick = (pieceId: number, isTarget: boolean) => {
		...
		if (isTarget) {
			...
			if (selectedPiece !== null) {
				...
				if (selectedPieceData && targetSlot && !targetSlot.isCorrect) {
					...
					if (selectedPieceData.skill.name === targetSlot.skill.name) {
						// Correct match!
						setTargetPieces((prev) =>
							prev.map((piece) =>
								piece.id === pieceId ? { ...piece, isCorrect: true } : piece,
							),
						);
						setPuzzlePieces((prev) =>
							prev.filter((p) => p.id !== selectedPiece),
						);
			...
			if (selectedPiece === pieceId) {
				...
				setHint(
					`Selected: ${puzzlePieces.find((p) => p.id === pieceId)?.skill.name} 📋`,
				);
			}
		}
	};
	...
	const renderSkillIcon = (skill: (typeof allSKills)[0], size = "normal") => {
		...
		return (
			<div className="flex flex-col items-center justify-center h-full">
				<IconComponent className={sizeClass} color={skill.color || "#00d8ff"} />
				<span
					className={`text-xs mt-1 text-center ${size === "small" ? "text-[8px]" : ""}`}
				>
					{skill.name.length > 8
			...
			{!gameStarted ? (
					...
					<div className="text-white">
						...
						<p className="text-sm text-white/60 mb-6">
							...
							<span className="text-emerald-400">
								⚡ Speed bonus: Finish faster for higher scores!
							</span>
						</p>
						<button
							type="button"
				...
				<div className="space-y-8">
					...
					<div className="space-y-4">
						<h4 className="text-white/90 font-semibold text-xs">
							Target Skills (Click to place selected piece):
						</h4>
						...
						>
							...
							<div className="space-y-2 text-white/80">
								...
								<p className="text-emerald-400">
									Time Bonus: +{score - (Math.floor(score / 100) - 1) * 100} pts
								</p>
							...
							<div className="text-sm text-white/60">
								{timer <= 30
									? "⚡ Lightning fast! Perfect time bonus!"
									: timer <= 60
										? "🚀 Great speed! Nice time bonus!"
