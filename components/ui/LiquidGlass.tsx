...
import { LiquidGlassProps, SceneElements } from './liquid-glass/types';
import {
...
export default function LiquidGlass({ className = '' }: LiquidGlassProps) {
  ...
  const { createRippleEffect, handleMouseMove, handleClick } = useMouseInteraction();
  ...
  useEffect(() => {
    if (!canvasRef.current || isReduced || typeof window === 'undefined') return;
    ...
    const liquidGlassPanels = createLiquidGlassPanels(glassContainer, renderSettings);
    ...
    const backgroundField = createBackgroundParticleField(scene, renderSettings);
    ...
    return () => {
      ...
      [
        ...
        ...(sceneRef.current.backgroundField ? [sceneRef.current.backgroundField] : []),
  ...
  }, [isReduced, createRippleEffect, handleMouseMove, handleClick, createRenderLoop, startFadeInAnimation]);

  // Fallback for reduced motion
  if (isReduced) {
    return (
      <div className={`${className} flex items-center justify-center bg-gradient-to-br from-slate-900/30 via-blue-900/20 to-indigo-900/30`}>
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div 
          ...
          style={{
            background: 'radial-gradient(ellipse at center, transparent 50%, rgba(15,23,42,0.08) 100%)'
          }} 
        />
      </div>
    </div>
