...
import { RenderSettings, FloatingElementUserData } from './types';
import {
  ...
  for (let i = 0; i < settings.panelCount; i++) {
    ...
    const panelMaterial = new THREE.ShaderMaterial({
      uniforms: {
        ...
        uDepth: { value: i * 0.3 },
        uOpacity: { value: materials.glass.opacity.base - i * materials.glass.opacity.decrement },
        uRefraction: { value: materials.glass.refractiveIndex },
        uThickness: { value: materials.glass.thickness.base + i * materials.glass.thickness.increment }
      },
...
export function createFloatingElements(
  ...
  for (let i = 0; i < settings.floatingElementCount; i++) {
    ...
    const userData: FloatingElementUserData = {
      ...
      floatSpeed: LIQUID_GLASS_CONFIG.animation.floatSpeed.min + 
                 Math.random() * (LIQUID_GLASS_CONFIG.animation.floatSpeed.max - 
                 LIQUID_GLASS_CONFIG.animation.floatSpeed.min),
      floatRadius: LIQUID_GLASS_CONFIG.animation.floatRadius.min + 
                  Math.random() * (LIQUID_GLASS_CONFIG.animation.floatRadius.max - 
...
export function createBackgroundParticleField(
  ...
  for (let i = 0; i < settings.particleCount; i++) {
    ...
    sizes[i] = materials.particles.sizeRange.min + 
               Math.random() * (materials.particles.sizeRange.max - materials.particles.sizeRange.min);
  }
