...
import { SceneElements } from './types';
import {
  rippleVertexShader,
...
export function useMouseInteraction() {
  ...
  const handleMouseMove = useCallback((
    ...
    sceneRef.current.targetMouse.x = (event.clientX / window.innerWidth) * 2 - 1;
    sceneRef.current.targetMouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
  ...
  const handleClick = useCallback((
    event: MouseEvent,
    sceneRef: React.MutableRefObject<SceneElements>,
    createRipple: (scene: THREE.Scene | undefined, x: number, y: number) => void
  ) => {
...
export function useRenderLoop() {
  ...
  const updateFloatingElements = useCallback((
    ...
    elements.forEach((element, index) => {
      ...
      element.position.x = userData.initialPosition.x + Math.sin(phase) * userData.floatRadius;
      element.position.y = userData.initialPosition.y + Math.cos(phase * 0.7) * userData.floatRadius * 0.5;
      element.position.z = userData.initialPosition.z + Math.sin(phase * 0.5) * userData.floatRadius * 0.3;
  ...
  const updateInteractiveOrbs = useCallback((
    ...
    orbs.forEach((orb, index) => {
      ...
      if (shouldUpdateExpensive) {
        ...
        const liquidX = Math.cos(baseTime + indexOffset) * 4 + 
                       Math.sin(baseTime * 0.7 + indexOffset) * 2;
        const liquidY = Math.sin(baseTime * 0.6 + indexOffset) * 3 + 
                       Math.cos(baseTime * 0.4 + indexOffset) * 1.5;
        const liquidZ = -2 - index * 1.2 + Math.sin(baseTime * 0.5 + indexOffset) * 1.5;
  ...
  const createRenderLoop = useCallback((
    ...
    return function renderLoop(timestamp: number) {
      ...
      if (sceneRef.current.glassContainer && shouldUpdateExpensive) {
        sceneRef.current.glassContainer.rotation.y = Math.sin(t * 0.08) * 0.015;
        sceneRef.current.glassContainer.rotation.x = Math.cos(t * 0.12) * 0.008;
      }
